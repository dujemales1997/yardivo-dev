...
if "yardivo-self-gate-admin-buttons-v2" in s:
    print("already patched")
    raise SystemExit(0)

anchor="function ensureWallAdmin(){"
if s.count(anchor)!=1:
    raise SystemExit(f"ensureWallAdmin anchor count {s.count(anchor)}")

helpers=r"""function setWallButtons(exists,busy=false){
...
start=s.index("async function loadWallQrState(){")
end=s.index("/* PORTA - Self Check-In queue */",start)
if start<0 or end<0:
    raise SystemExit("Self Gate admin function range not found")

replacement=r"""async function loadWallQrState(){
...
s=s.replace("</body>",'<script id="yardivo-self-gate-admin-buttons-v2">window.YARDIVO_DEV_BUILD="20260923-v5.8.3-self-gate-admin-buttons-v16";</script>\n</body>',1)
p.write_text(s,encoding="utf-8")
print("Self Gate Admin QR controls patched")
